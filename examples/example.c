...
static int filter_older_than_30(const csv_row *row, void *userdata) {
    csv_csv *csv = (csv_csv *)userdata;
...
static void validate_file(const char *path, char delim, const char *label) {
    ...
    csv_csv *c = delim ? csv_open_with_delim(path, delim, 1) : csv_open(path, 1);
    ...
    csv_col_iter cit = csv_column_by_name(c, "salary");
    ...
    /* --- filter: age > 30 --------------------------------------------- */
    int filtered = 0;
    csv_filter_iter fit = csv_filter(c, filter_older_than_30, c);
    while (csv_filter_next(&fit)) {
        csv_row *fr = &fit._base.row;
...
static void test_mem(void) {
    ...
    csv_csv *c = csv_open_mem(buf, strlen(buf), ',', 1);
